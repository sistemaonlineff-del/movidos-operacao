...
export default function Funcionarios() {
  ...
  const save = async (event: FormEvent) => {
    ...
    for (const removed of removedDependents) {
      const removal = await supabase.from('employee_dependents').update({ is_active: false, deactivated_reason: removed.reason, deactivated_at: new Date().toISOString(), deactivated_by: auth.user?.id ?? null }).eq('id', removed.id)
      if (removal.error) { setMessage(removal.error.message); setSaving(false); return }
    }
