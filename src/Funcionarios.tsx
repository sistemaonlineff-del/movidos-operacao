...
const blank = (): Employee => ({ ...Object.fromEntries(fields.map(key => [key, ''])), access_role: 'operador', employment_status: 'ativo', transport_voucher: false })
...
export default function Funcionarios() {
  ...
  const [removedDependents, setRemovedDependents] = useState<Array<{ id: string; reason: string }>>([])
  ...
  const load = async () => {
    ...
    if (currentProfile?.role === 'admin' || can('funcionarios_view')) {
  ...
  const reset = (clearMessage = true) => { setEditing(null); setValue(blank()); setDependents([]); setRemovedDependents([]); if (clearMessage) setMessage('') }
  const edit = async (employee: Employee) => {
    ...
    setRemovedDependents([])
    const { data, error } = await supabase!.from('employee_dependents').select('*').eq('employee_id', employee.id).eq('is_active', true).order('full_name')
  ...
  const save = async (event: FormEvent) => {
    ...
    if (savingRef.current) return
    setSaveError(true)
    if (!can('funcionarios_manage')) { setMessage('Você não tem permissão para salvar funcionários.'); return }
    ...
    if (state && !/^[A-Z]{2}$/.test(state)) { setMessage('Informe a UF com duas letras ou deixe o campo vazio.'); return }
    const monthlyHours = value.monthly_hours == null || value.monthly_hours === '' ? null : Number(value.monthly_hours)
    if (monthlyHours != null && (!Number.isFinite(monthlyHours) || monthlyHours < 0 || monthlyHours >= 100000)) { setMessage('Informe uma carga horária mensal válida.'); return }
    if (!['operador', 'financeiro'].includes(value.access_role) || !['ativo', 'inativo', 'desligado'].includes(value.employment_status)) { setMessage('Selecione a permissão e a situação do funcionário.'); return }
    for (const dependent of dependents) {
      if (!dependent.full_name.trim() || !dependent.relationship.trim() || (onlyDigits(dependent.cpf) && onlyDigits(dependent.cpf).length !== 11)) { setMessage('Preencha nome e parentesco de cada dependente; o CPF pode ficar vazio ou deve ter 11 dígitos.'); return }
    }
    ...
    try {
      ...
      if (authError || !auth.user) throw new Error('Entre novamente no sistema antes de salvar o funcionário.')
      const payload: Employee = Object.fromEntries(fields.map(field => [field, String(value[field] ?? '').trim() || null]))
      Object.assign(payload, { cpf: onlyDigits(value.cpf), personal_email: value.personal_email.trim().toLowerCase(), state: state || null, monthly_hours: monthlyHours, access_role: value.access_role, employment_status: value.employment_status, transport_voucher: value.transport_voucher === true })
      if (!editing) payload.created_by = auth.user.id
      ...
      if (!response.data?.id) throw new Error('O banco não confirmou o salvamento do funcionário.')
      const employeeId = response.data.id
      employeeSaved = true
      setEditing(employeeId)
      for (const removed of removedDependents) {
        const removal = await supabase.from('employee_dependents').update({ is_active: false, deactivated_reason: removed.reason, deactivated_at: new Date().toISOString(), deactivated_by: auth.user.id }).eq('id', removed.id).eq('employee_id', employeeId).select().single()
        if (removal.error) throw removal.error
        if (!removal.data?.id || removal.data.is_active !== false) throw new Error('O banco não confirmou a desativação do dependente.')
        setRemovedDependents(current => current.filter(item => item.id !== removed.id))
      }
      for (const [index, dependent] of dependents.entries()) {
        ...
        const result = dependent.id ? await supabase.from('employee_dependents').update(dependentPayload).eq('id', dependent.id).eq('employee_id', employeeId).select().single() : await supabase.from('employee_dependents').insert(dependentPayload).select().single()
        if (result.error) throw result.error
        if (!result.data?.id) throw new Error('O banco não confirmou o salvamento do dependente.')
        setDependents(current => current.map((item, position) => position === index ? { ...item, id: result.data.id } : item))
      }
      reset(false); setSaveError(false)
      setMessage(editing ? 'Funcionário atualizado com sucesso.' : 'Funcionário cadastrado com sucesso.')
      await load()
    } catch (error) {
      const detail = error?.code === '23505' ? 'Já existe um funcionário com este CPF ou e-mail.' : error?.code === '42501' ? 'Sua conta não tem permissão para esta gravação. Peça a um administrador para conferir o acesso a Funcionários.' : error?.message || 'Confira a conexão e tente novamente.'
      setMessage(`${employeeSaved ? 'Funcionário salvo, mas há dependentes pendentes. Os dados foram mantidos para tentar novamente.' : 'Não foi possível salvar o funcionário.'} ${detail}`)
      setSaveError(true)
  ...
  const removeDependent = (dependent: Dependent, index: number) => {
    const reason = window.prompt(`Informe obrigatoriamente o motivo para remover ${dependent.full_name || 'este dependente'}:`)?.trim()
    if (!reason) { setMessage('A observação é obrigatória. O dependente não foi removido.'); return }
    if (dependent.id) setRemovedDependents(current => [...current, { id: dependent.id!, reason }])
    setDependents(current => current.filter((_, position) => position !== index))
  }

  if (loading) return <section className="card">Carregando funcionários...</section>
  if (!can('funcionarios_view')) return <section className="card employee-locked"><p className="eyebrow">ACESSO RESTRITO</p><h2>Área de Funcionários</h2><p>Peça a um administrador para liberar o módulo de Funcionários.</p></section>
  return <section className="employees"><div className="employee-title"><div><p className="eyebrow">GESTÃO DE PESSOAS</p><h2>{editing ? 'Editar funcionário' : 'Funcionários'}</h2><p>Dados pessoais, vínculo de trabalho, dependentes e acesso ao sistema.</p></div>{editing && <button className="secondary" type="button" disabled={saving} onClick={reset}>Novo cadastro</button>}</div>
    {can('funcionarios_manage') && <form className="employee-form" onSubmit={save}>
      <fieldset className="employee-fields" disabled={saving}>
