...
export default function LastMileCadastro() {
  ...
  const submit=async(event:FormEvent)=>{event.preventDefault(); if(!supabase || saving)return; if(!values.name.trim()){setMessage("Informe o nome.");return} setSaving(true);setMessage("");
    const money=(value:string)=>Number(value.replace(/[^0-9,.-]/g,"").replace(",","."))||0;
    const payload:any={...values, registration_type:"last_mile", weekly_package_value:money(values.weekly_package_value), sunday_holiday_package_value:money(values.sunday_holiday_package_value), work_days:values.work_days.join(", ")};
    const query=id ? supabase.from("drops").update(payload).eq("id",id) : supabase.from("drops").insert({...payload,status:"INTERESSADO"});
    const {data,error}=await query.select().single(); setSaving(false);
    if(error){setMessage(error.message);return} setId(data.id);setMessage("Cadastro Last Mile salvo. Agora você pode anexar fotos, contratos e distratos.");
  };
  ...
  {message&&<p className="form-message" role="status">{message}</p>}<div className="modal-actions"><button type="button" onClick={()=>nav("/cadastros/last-mile")}>Cancelar</button><button className="primary compact" disabled={saving}>{saving?"Salvando…":id?"Salvar alterações":"Salvar cadastro Last Mile"}</button></div></form></section>
}
